...
```
```{R}
lambdas <- 10^seq(6, -2, by = -.5)

cv_fit <- cv.glmnet(sparse_matrix, response, alpha = 0, lambda = lambdas)
```
...
```{R}
cv_fit2 <- cv.glmnet(sparse_matrix, response, alpha = 0, lambda = lambdas2)
```
...
```
```{R}
lambdas3 = 10^seq(-4, -10, by = -.5)
cv_fit3 <- cv.glmnet(sparse_matrix, response, alpha = 0, lambda = lambdas3)
```
